#!/usr/bin/env python3
"""Red agent - aggressive: full throttle, late to lift for traffic.
...
# personality
LOOK_AHEAD = 0.62    # scanline height, as a fraction of the frame
DEADZONE = 5.0       # px of steering slack before correcting
EDGE = 0.72          # lane offset past which we haul it back regardless
BLOB = 150           # saturated pixels ahead that count as "car in the way"
CLEARANCE = 34.0     # px to leave when going round a car
LIFT = False         # red keeps its foot in
...
def main():
    ...
    while True:
        ...
        if state['status'] == 'race_over':
            ...
            print(f'red: race over - winner {res.get("winner")} '
